function getGearBoxByteData(int8Array, byteGearBox, getRead)
{
...
function setGearBoxValues(int8Array, byteGearBox, setRead)
{
  for (var ii = 0; ii < byteGearBox.byteGears.length; ++ii )
  {
    var toothList;
...
function getGearBoxByteGear(name, byteGearBox)
{
  for (var ii = 0; ii < byteGearBox.byteGears.length; ++ii )
    if (byteGearBox.byteGears[ii].name == name) return byteGearBox.byteGears[ii];
  throw name + ' not found';
}
function getGearBoxByteTooth(name, byteGear, getRead)
{
    var toothList;
    if (getRead)
    {
...
function printGearBoxData(byteGearBox, printRead)
{
  var byteLength;
  var byteLengthLabel;
  if (printRead)
